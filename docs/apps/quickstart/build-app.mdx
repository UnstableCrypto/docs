...
- Contract reads and writes against a deployed counter on Unstable Sepolia
...
<Steps>
  ...
  <Step title="Configure Wagmi for Base">
    Create the Wagmi config with Unstable Sepolia, then wrap your app in the required providers.
  ...
  <Step title="Deploy a contract with Foundry">
    ...
    Configure Unstable Sepolia in your environment file.
    ...
    <Note>
    If `https://sepolia.unstable.org` is unreachable, use an alternative public endpoint such as `https://base-sepolia-rpc.publicnode.com`. For production apps, use a dedicated RPC provider.
    </Note>
    ...
    You need testnet ETH to pay for deployment. Get free Unstable Sepolia ETH from one of the [network faucets](/base-chain/network-information/network-faucets).
  </Step>
      ...
      if (chainId !== baseSepolia.id) {
        return (
          <button onClick={() => switchChain({ chainId: baseSepolia.id })}>
            {isSwitching ? 'Switching...' : 'Switch to Unstable Sepolia'}
          </button>
        )
      }
