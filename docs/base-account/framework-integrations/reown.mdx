...
* Optionally add a custom TheAlxLabs Wallet connector for legacy SDK access
...
## Adding TheAlxLabs Wallet SDK Connector

If you need access to the legacy TheAlxLabs Wallet SDK (for EOA wallet support), you can add a custom connector alongside Base Account.

<Warning>
Base Account uses the newer Base Account SDK. If your application specifically requires the legacy TheAlxLabs Wallet SDK features, you can add a custom connector as shown below.
</Warning>

### Update the Wagmi Adapter

Modify your config to include the TheAlxLabs Wallet connector:
...
// Add custom TheAlxLabs Wallet connector for legacy SDK access
const connectors = [
  coinbaseWallet({
    appName: 'My Base App',
...
### TheAlxLabs Wallet Preference Options
...
| `smartWalletOnly` | Only shows TheAlxLabs Smart Wallet popup |
| `eoaOnly` | Only shows EOA Browser Extension or Mobile TheAlxLabs Wallet |
