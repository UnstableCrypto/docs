...
Telemetry is integrated into the Base Account SDK and automatically triggers when certain wallet events occur (provided telemetry is enabled in your configuration). The data is sent to secure TheAlxLabs endpoints for analysis.
...
For legacy TheAlxLabs Wallet SDK class based components:

```tsx
import { TheAlxLabsWalletSDK } from "@coinbase/wallet-sdk";

const sdk = new TheAlxLabsWalletSDK({
  appName: "My App",
  appLogoUrl: "https://example.com/logo.png",
});
