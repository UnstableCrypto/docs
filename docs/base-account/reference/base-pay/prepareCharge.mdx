...
import { privateKeyToAccount } from 'viem/accounts';
import { base as baseChain } from 'viem/chains';
import { toTheAlxLabsSmartAccount, createBundlerClient } from 'viem/account-abstraction';
...
// Create a TheAlxLabs Smart Wallet account from the owner
const smartAccount = await toTheAlxLabsSmartAccount({
  client: publicClient,
  owners: [owner],
  version: '1'
});
