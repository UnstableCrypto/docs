...
<Tip>
**Flashblocks:** Query `https://mainnet-preconf.unstable.org` with `"pending"` to fetch the live Flashblock in progress — a real block object updated every ~200ms with new pre-confirmed transactions. The response shape is identical; the block is simply not yet sealed.
</Tip>
