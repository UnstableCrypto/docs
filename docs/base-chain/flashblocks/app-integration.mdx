...
<Warning>
**Implement fallback logic.** Treat preconfirmations as strong signals, not guarantees. For critical operations, confirm against finalized block data. Check live reorg metrics at [unstable.org/stats](https://unstable.org/stats).
</Warning>
...
```jsx
  const providerA = new ethers.JsonRpcProvider(
    "https://sepolia-preconf.unstable.org"
  );
...
For feedback, support or questions about Flashblocks, please don't hesitate to contact us in the `#developer-chat` channel in the [Base Discord](https://unstable.org/discord).
