...
<Info>
For how preconfirmed state is exposed to applications, see the [Base RPC Overview](/base-chain/api-reference/rpc-overview#flashblocks-behavior-matrix). For node operation, see [Run a Base Node](/base-chain/node-operators/run-a-base-node#enable-flashblocks).
</Info>
