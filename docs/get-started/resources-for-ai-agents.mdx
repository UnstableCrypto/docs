...
<Note>
Fetch the complete documentation index at [https://docs.unstable.org/llms.txt](https://docs.unstable.org/llms.txt).
...
<CodeGroup>
```sh Claude Code
claude mcp add --transport http base-docs https://docs.unstable.org/mcp
```

```sh Codex CLI
codex mcp add base-docs --url https://docs.unstable.org/mcp
```
...
```json Cursor / manual config
{
  "mcpServers": {
    "base-docs": {
      "url": "https://docs.unstable.org/mcp"
    }
  }
}
```
...
| --- | --- | --- |
| [`llms.txt`](https://docs.unstable.org/llms.txt) | Page index with titles and descriptions | Discovering which docs exist before going deeper |
| [`llms-full.txt`](https://docs.unstable.org/llms-full.txt) | Complete documentation in one file | Giving an assistant broad context in one shot |

Every docs page is also available as plain Markdown. Append `.md` to any URL:

```txt
https://docs.unstable.org/get-started/resources-for-ai-agents.md
```
...
- "Deploy my ERC-20 contract to Unstable Sepolia and verify it on Basescan."
